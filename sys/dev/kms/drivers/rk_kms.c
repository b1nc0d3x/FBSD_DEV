    ...
    const struct drm_display_mode *mode)
{
	...
	/*
	 * Re-arm DP audio.  auto_bringup_default reloads the MHDP and
	 * enable_mode reprograms the framer; both leave the audio
	 * packetizer zeroed — AUDIO_SRC_CNTL, SMPL2PKT_CNTL and
	 * PIF_PKT_ALLOC all read back 0, so the sink sits on a trained
	 * link with no audio packets.  usbc_bringup is the only other
	 * caller and it fires only on a fresh fusb302 attach sequence,
	 * which means the first modeset after boot (Xorg's SETCRTC)
	 * silently killed audio until the cable was replugged.  Ordering
	 * matches usbc_bringup: only after video is active, since
	 * audio_start against an untrained link wedges cdn_dp's mailbox.
	 * Non-fatal — a modeset that carries video is still a success.
	 */
	error = rk_cdn_dp_audio_start(2, 48000, 16);
	if (error != 0)
		DPRINTF(sc, "DP audio re-arm failed: %d\n", error);
...
rk_kms_attach(device_t dev)
{
	...
	device_printf(dev, "registered (Phase 9h: VOP + Cadence MHDP DP TX "
	    "wired behind commit_modeset sysctl, default off)\n");
	return (0);
}
