...
kms_file_dtor(void *data)
{
	...
	if (dev->open_count > 0)
		dev->open_count--;
	/*
	 * Do NOT auto-promote another file to master here.  Earlier we
	 * did — TAILQ_FOREACH picks the OLDEST remaining fd, which is
	 * typically the SDDM parent / a kioworker / etc., not the user's
	 * new session compositor.  Handing master to the wrong file
	 * means the user's kwin_wayland gets EACCES on every atomic
	 * commit (see the DRM_MASTER gate above the ioctl dispatch), so
	 * the display never updates and the screen stays dark after a
	 * greeter → user-session handoff (SDDM without autologin).
	 *
	 * The SET_MASTER handler's "open seat" branch (drm_ioctl.c
	 * SET_MASTER case, "No file on this device is master → allow"
	 * clause) is exactly what handles the "stranded fd" scenario
	 * this used to worry about — kwin can seize the empty seat with
	 * an unprivileged SET_MASTER after the previous master closes.
	 */
