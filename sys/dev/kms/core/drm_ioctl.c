    ...
    int fflag __unused, struct thread *td)
{
	...
	/*
	 * Table-driven dispatch — see drm_ioctl_table.[ch].  Returns >= 0
	 * on hit (errno from a real handler, or 0 on success), or -1 to
	 * fall through to the legacy switch below.  When the tunable
	 * dev.kms.table_dispatch=0 (default) the call returns -1 without
	 * doing any work, so the switch runs unchanged.  When enabled,
	 * DELEGATE-marked slots enforce the permission gate then return
	 * -1 so the switch sees a pre-vetted request — no duplicate check
	 * cost, and no per-slot switch cleanup blocked on the migration.
	 */
	error = kms_ioctl_table_dispatch(file, cmd, data, td);
	if (error >= 0)
		return (error);
	 ...
	 */
	/*
	 * DRM master vs auth split, matching Linux DRM_MASTER / DRM_AUTH flags.
	 *
	 * MODESET / display-affecting ops require master — they change what
	 * the display shows or how it's read, so only one client can drive
	 * them at a time.  BUFFER-add ops (ADDFB, ADDFB2, CREATEPROPBLOB /
	 * DESTROYPROPBLOB) require only auth — a Wayland compositor routinely
	 * opens card0 twice (a master fd for SETCRTC/PAGE_FLIP/ATOMIC + a
	 * non-master fd for rendering/ADDFB2).  Gating ADDFB2 as master-only
	 * made kwin's non-master render fd hit EACCES on every buffer create.
	 *
	 * Ops that read scanout state (GETFB, GETFB2, OBJ_GETPROPERTIES) or
	 * write per-object properties (OBJ_SETPROPERTY) or invalidate scanout
	 * (RMFB, DIRTYFB) are MASTER — matches Linux drm_ioctls[].  Without
	 * this, a non-master video-group user can read the compositor's live
	 * front buffer (GETFB), IPI-storm the box (DIRTYFB), invalidate the
	 * compositor's fb_id (RMFB), or flip video source on multi-output
	 * drivers (OBJ_SETPROPERTY → rk_kms video_src).
	 *
	 * We don't require explicit AUTH_MAGIC for the auth-flagged ops —
	 * any unprivileged file that opened card0 has already passed the
	 * video group check on the cdev.  Real auth (per-magic-token gating)
	 * is only useful when the master needs to whitelist specific clients.
	 */
	if (!file->is_master) {
		switch (cmd) {
		case DRM_IOCTL_MODE_SETCRTC:
		...
		case DRM_IOCTL_MODE_GETFB2:
			return (EACCES);
		/*
		 * NOTE: DRM_IOCTL_MODE_OBJ_GETPROPERTIES is NOT in this
		 * list.  Linux relaxed it from DRM_MASTER to
		 * DRM_RENDER_ALLOW so Mesa can query plane format
		 * modifiers from the render node.  Property reads return
		 * metadata only — no scanout-fb exposure.
		 */
		}
	}
	...
	case DRM_IOCTL_MODE_LIST_LESSEES: {
		return (kms_tbl_mode_list_lessees(file, data, td));
	}
	case DRM_IOCTL_MODE_GET_LEASE: {
		return (kms_tbl_mode_get_lease(file, data, td));
	}
	case DRM_IOCTL_MODE_CREATE_LEASE:
		return (kms_tbl_mode_lease_unsupported(file, data, td));
	case DRM_IOCTL_MODE_REVOKE_LEASE:
	case DRM_IOCTL_SET_MASTER: {
		return (kms_tbl_set_master(file, data, td));
	}
	case DRM_IOCTL_DROP_MASTER:
		return (kms_tbl_drop_master(file, data, td));
	case DRM_IOCTL_GET_MAGIC: {
		return (kms_tbl_get_magic(file, data, td));
	}
	case DRM_IOCTL_AUTH_MAGIC: {
		return (kms_tbl_auth_magic(file, data, td));
	}
	...
	case DRM_IOCTL_PRIME_FD_TO_HANDLE:
		return (kms_ioctl_prime_fd_to_handle(file,
		    (struct drm_prime_handle *)data));
	case DRM_IOCTL_MODE_SETGAMMA:
		return (kms_tbl_mode_setgamma(file, data, td));
	case DRM_IOCTL_MODE_DIRTYFB:
		return (kms_tbl_mode_dirtyfb(file, data, td));
	case DRM_IOCTL_MODE_ADDFB:
	...
	case DRM_IOCTL_IRQ_BUSID:
		return (kms_tbl_irq_busid(file, data, td));
	case DRM_IOCTL_MODE_SETPROPERTY:
		return (kms_tbl_mode_setproperty(file, data, td));
	case DRM_IOCTL_MODE_SETPLANE:
		return (kms_tbl_mode_setplane(file, data, td));
	}
	/*
 ...
 * Table adapters.
 *
 * Each one gives the ioctl table's uniform (file, data, td) signature to
 * an implementation that takes a typed pointer.  Named kms_tbl_* because
 * most of the implementations already own the kms_ioctl_* name.
 *
 * The matching switch cases stay put.  With dev.kms.table_dispatch=0 the
 * switch is still the only path; a case is deleted only once the tunable
 * has defaulted on for a while, per the order in drm_ioctl_table.h.
 */

int
kms_tbl_version(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (drm_ioctl_version(file, (struct drm_version *)data));
}

int
kms_tbl_get_unique(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (drm_ioctl_get_unique(file, (struct drm_unique *)data));
}

int
kms_tbl_set_version(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (drm_ioctl_set_version(file, (struct drm_set_version *)data));
}

int
kms_tbl_get_cap(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (drm_ioctl_get_cap(file, (struct drm_get_cap *)data));
}

int
kms_tbl_mode_getresources(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getresources(file, (struct drm_mode_card_res *)data));
}

int
kms_tbl_mode_getcrtc(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getcrtc(file, (struct drm_mode_crtc *)data));
}

int
kms_tbl_mode_getencoder(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getencoder(file, (struct drm_mode_get_encoder *)data));
}

int
kms_tbl_mode_getconnector(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getconnector(file, (struct drm_mode_get_connector *)data));
}

int
kms_tbl_mode_getplane_resources(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getplane_resources(file, (struct drm_mode_get_plane_res *)data));
}

int
kms_tbl_mode_getplane(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getplane(file, (struct drm_mode_get_plane *)data));
}

int
kms_tbl_mode_create_dumb(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_create_dumb(file, (struct drm_mode_create_dumb *)data));
}

int
kms_tbl_mode_map_dumb(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_map_dumb(file, (struct drm_mode_map_dumb *)data));
}

int
kms_tbl_mode_destroy_dumb(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_destroy_dumb(file, (struct drm_mode_destroy_dumb *)data));
}

int
kms_tbl_gem_close(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_gem_handle_delete(file, ((struct drm_gem_close *)data)->handle));
}

int
kms_tbl_prime_handle_to_fd(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_prime_handle_to_fd(file, (struct drm_prime_handle *)data));
}

int
kms_tbl_prime_fd_to_handle(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_prime_fd_to_handle(file, (struct drm_prime_handle *)data));
}

int
kms_tbl_mode_addfb(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_addfb(file, (struct drm_mode_fb_cmd *)data));
}

int
kms_tbl_mode_addfb2(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_addfb2(file, (struct drm_mode_fb_cmd2 *)data));
}

int
kms_tbl_mode_rmfb(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_rmfb(file, (uint32_t *)data));
}

int
kms_tbl_mode_closefb(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_closefb(file, (struct drm_mode_closefb *)data));
}

int
kms_tbl_mode_getfb(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getfb(file, (struct drm_mode_fb_cmd *)data));
}

int
kms_tbl_mode_getfb2(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getfb2(file, (struct drm_mode_fb_cmd2 *)data));
}

int
kms_tbl_mode_setcrtc(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_setcrtc(file, (struct drm_mode_crtc *)data));
}

int
kms_tbl_mode_page_flip(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_page_flip(file, (struct drm_mode_crtc_page_flip *)data));
}

int
kms_tbl_set_client_cap(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_set_client_cap(file, (struct drm_set_client_cap *)data));
}

int
kms_tbl_mode_getproperty(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getproperty(file, (struct drm_mode_get_property *)data));
}

int
kms_tbl_mode_obj_getproperties(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_obj_getproperties(file, (struct drm_mode_obj_get_properties *)data));
}

int
kms_tbl_mode_obj_setproperty(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_obj_setproperty(file, (struct drm_mode_obj_set_property *)data));
}

int
kms_tbl_mode_createpropblob(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_createpropblob(file, (struct drm_mode_create_blob *)data));
}

int
kms_tbl_mode_destroypropblob(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_destroypropblob(file, (struct drm_mode_destroy_blob *)data));
}

int
kms_tbl_mode_getpropblob(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_getpropblob(file, (struct drm_mode_get_blob *)data));
}

int
kms_tbl_mode_atomic(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_atomic(file, (struct drm_mode_atomic *)data));
}

int
kms_tbl_syncobj_create(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_syncobj_create(file, (struct drm_syncobj_create *)data));
}

int
kms_tbl_syncobj_destroy(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_syncobj_destroy(file, (struct drm_syncobj_destroy *)data));
}

int
kms_tbl_syncobj_wait(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_syncobj_wait(file, (struct drm_syncobj_wait *)data));
}

int
kms_tbl_syncobj_reset(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_syncobj_reset(file, (struct drm_syncobj_array *)data));
}

int
kms_tbl_syncobj_signal(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_syncobj_signal(file, (struct drm_syncobj_array *)data));
}

int
kms_tbl_syncobj_handle_to_fd(struct drm_file *file, void *data, struct thread *td)
{
	return (kms_ioctl_syncobj_handle_to_fd(td, file, (struct drm_syncobj_handle *)data));
}

int
kms_tbl_wait_vblank(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_wait_vblank(file, (union drm_wait_vblank *)data));
}

int
kms_tbl_mode_cursor(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_cursor(file, (struct drm_mode_cursor *)data));
}

int
kms_tbl_mode_cursor2(struct drm_file *file, void *data, struct thread *td __unused)
{
	return (kms_ioctl_mode_cursor2(file, (struct drm_mode_cursor2 *)data));
}

/*
 * Hoisted handlers.
 *
 * These ten had real bodies in the switch rather than a one-line call, so
 * the body moved here and the switch case became a call to it.  One copy
 * of the code, and both dispatch paths run it.
 */

int
kms_tbl_mode_list_lessees(struct drm_file *file, void *data, struct thread *td __unused)
{
	/*
...
int
kms_tbl_mode_get_lease(struct drm_file *file, void *data, struct thread *td __unused)
{
	/*
...
int
kms_tbl_mode_lease_unsupported(struct drm_file *file, void *data, struct thread *td __unused)
{
	/*
...
int
kms_tbl_set_master(struct drm_file *file, void *data, struct thread *td __unused)
{
	/*
	 ...
	 *
	 * Privilege gate.  Cases in priority order:
	 *   1. Caller already holds master → no-op, allow.
	 *   2. No file on this device is master → open seat, allow
	 *      (first-come-first-serve — matches the implicit
	 *      "first opener becomes master" behaviour in
	 *      kms_file_open; without this, a Wayland compositor
	 *      spawning after Xorg exits could never take master
	 *      unless it runs privileged, since PRIV_DRIVER gates
	 *      the general case).
	 *   3. Some other file is master → require PRIV_DRIVER
	 *      (equivalent to Linux CAP_SYS_ADMIN).  Prevents a
	 *      concurrent opener from stealing master from the
	 *      active Xorg / kwin_wayland session mid-flight.
	 */
	...
	if (!file->is_master) {
		TAILQ_FOREACH(peer, &file->dev->files, link) {
			if (peer != file && peer->is_master) {
				have_master = true;
				break;
			}
		}
	}
	if (!file->is_master && have_master) {
		sx_xunlock(&file->dev->dev_lock);
		error = priv_check(td, PRIV_DRIVER);
		if (error != 0)
			return (error);
		sx_xlock(&file->dev->dev_lock);
		/* Only reach here if we survived the priv_check;
		 * proceed to seize master, demoting everyone else. */
	}
	TAILQ_FOREACH(peer, &file->dev->files, link) {
		if (peer != file)
...
int
kms_tbl_drop_master(struct drm_file *file, void *data, struct thread *td __unused)
{
	sx_xlock(&file->dev->dev_lock);
...
int
kms_tbl_get_magic(struct drm_file *file, void *data, struct thread *td __unused)
{
	/*
...
int
kms_tbl_auth_magic(struct drm_file *file, void *data, struct thread *td __unused)
{
	/*
...
int
kms_tbl_mode_setgamma(struct drm_file *file, void *data, struct thread *td __unused)
{
	/*
...
int
kms_tbl_mode_dirtyfb(struct drm_file *file, void *data, struct thread *td __unused)
{
	/*
 ...
 * IRQ_BUSID.
 *
 * A UMS-era call: userspace asked the kernel which IRQ a PCI bus id was
 * wired to, back when the X server drove interrupts itself.  Linux
 * answers it only for DRIVER_LEGACY drivers and returns EOPNOTSUPP for
 * everything modern.  We are modeset-only, so that is the honest answer
 * -- and a definite one beats falling through to the driver hook and
 * surfacing as ENOTTY, which reads to userspace as "no such ioctl"
 * rather than "not applicable to this driver".
 */
int
kms_tbl_irq_busid(struct drm_file *file __unused, void *data __unused,
...
int
kms_tbl_mode_setproperty(struct drm_file *file, void *data,
    struct thread *td __unused)
{
	return (kms_ioctl_mode_setproperty(file,
	    (struct drm_mode_connector_set_property *)data));
}

int
kms_tbl_mode_setplane(struct drm_file *file, void *data,
    struct thread *td __unused)
{
	return (kms_ioctl_mode_setplane(file, (struct drm_mode_set_plane *)data));
}
