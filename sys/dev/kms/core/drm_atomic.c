...
/*
 * Legacy connector property write.
 *
 * The connector-only ancestor of MODE_OBJ_SETPROPERTY: same value and
 * property id, with the object implied to be a connector.  Xorg's
 * modesetting driver still uses it for DPMS and scaling-mode on
 * pre-atomic paths.  Rather than duplicate the property lookup, refcount
 * and driver-callback handling, translate to the general form and let
 * kms_ioctl_mode_obj_setproperty do the work -- which is how Linux
 * arranges it too.
 */
int
kms_ioctl_mode_setproperty(struct drm_file *file,
    struct drm_mode_connector_set_property *r)
{
	struct drm_mode_obj_set_property o;

	if (file == NULL || r == NULL)
		return (EINVAL);
	o.value = r->value;
	o.prop_id = r->prop_id;
	o.obj_id = r->connector_id;
	o.obj_type = DRM_MODE_OBJECT_CONNECTOR;
	return (kms_ioctl_mode_obj_setproperty(file, &o));
}

/*
 * Legacy single-plane update.
 *
 * There is no legacy plane-programming hook -- drm_plane_funcs carries
 * only destroy and atomic_check -- so the only way to actually move a
 * plane is the atomic path.  Build a state holding just this plane,
 * validate it the same way MODE_ATOMIC does, and commit.  Linux takes
 * the same route for atomic drivers via drm_atomic_helper_set_plane.
 *
 * fb_id == 0 means "turn this plane off": no framebuffer, no CRTC, and
 * the rectangles zeroed.  Otherwise both the CRTC and the framebuffer
 * must resolve, and the CRTC has to be one this plane can actually feed
 * (possible_crtcs).
 *
 * Reference handling follows the MODE_ATOMIC contract: ids resolved into
 * a plane state are owned by the state, and kms_atomic_state_free ->
 * kms_atomic_state_release_refs drops them.  So nothing here puts fb or
 * crtc by hand; the plane_id lookup is ours to release and is the only
 * one we drop.
 */
int
kms_ioctl_mode_setplane(struct drm_file *file, struct drm_mode_set_plane *r)
{
	struct drm_device *dev;
	struct drm_mode_config *mc;
	struct drm_mode_object *plane_obj, *o;
	struct drm_plane *plane;
	struct drm_atomic_state *state;
	struct drm_plane_state *ps;
	struct drm_crtc *crtc;
	...
	/*
	 * Linux ignores this field, but an unknown bit reaching a driver
	 * is the failure mode MODE_ATOMIC's flag mask exists to prevent,
	 * so reject rather than silently drop.  If a caller ever turns up
	 * setting a stray bit, this is the line to relax.
	 */
	if (r->flags != 0)
		return (EINVAL);
	if (mc->funcs == NULL || mc->funcs->atomic_commit == NULL)
		return (EOPNOTSUPP);

	plane_obj = kms_mode_object_find(dev, r->plane_id,
	    DRM_MODE_OBJECT_PLANE);
	if (plane_obj == NULL)
		return (ENOENT);
	plane = __containerof(plane_obj, struct drm_plane, base);

	state = kms_atomic_state_alloc(dev);
	ps = kms_atomic_get_plane_state(state, plane);
	if (ps == NULL) {
		error = EINVAL;
		goto out;
	...
	if (r->fb_id == 0) {
		...
		o = kms_atomic_resolve_id(dev, r->crtc_id,
		    DRM_MODE_OBJECT_CRTC, &error);
		if (o == NULL) {
			if (error == 0)
				error = ENOENT;
		...
		crtc = __containerof(o, struct drm_crtc, base);
		if ((plane->possible_crtcs & (1u << crtc->index)) == 0) {
			kms_mode_object_put(o);
			error = EINVAL;
			goto out;
		}
		ps->crtc = crtc;

		o = kms_atomic_resolve_id(dev, r->fb_id, DRM_MODE_OBJECT_FB,
		    &error);
		if (o == NULL) {
			if (error == 0)
				error = ENOENT;
		...
		ps->fb = __containerof(o, struct drm_framebuffer, base);

		ps->crtc_x = r->crtc_x;
		ps->crtc_y = r->crtc_y;
		ps->crtc_w = r->crtc_w;
		ps->crtc_h = r->crtc_h;
		/* src_* are 16.16 fixed point in the uapi and in the state. */
		ps->src_x = r->src_x;
		ps->src_y = r->src_y;
		ps->src_w = r->src_w;
		ps->src_h = r->src_h;
	...
	error = kms_atomic_check_planes(state);
	if (error == 0 && mc->funcs->atomic_check != NULL)
		error = mc->funcs->atomic_check(dev, state);
	if (error == 0)
		error = mc->funcs->atomic_commit(dev, state, false);
out:
	kms_atomic_state_free(state);
	kms_mode_object_put(plane_obj);
	return (error);
}
